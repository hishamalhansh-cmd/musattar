...
def clear_all_data_once():
    """يمسح بيانات المستخدمين مرة واحدة فقط بعد أول تشغيل لهذا الملف.
    يحافظ على حساب الأدمن admin / 1234 داخل جدول admin_settings.
    """
    marker_path = os.path.join(APP_DATA_DIR or BASE_DIR, ".musattar_data_cleared_once")

    if os.path.exists(marker_path):
        print("تخطي مسح البيانات: تم المسح سابقاً")
        return
    ...
    try:
            ...
            if USING_POSTGRES:
                ...
                for table_name in [
                    ...
                    try:
                        ...
                        print(f"تعذر مسح جدول {table_name}:", table_error)
        ...
        with open(marker_path, "w", encoding="utf-8") as marker_file:
            marker_file.write(datetime.datetime.now().isoformat())

        print("تم مسح بيانات التطبيق القديمة مرة واحدة بنجاح")
    except Exception as e:
        print("خطأ أثناء مسح بيانات التطبيق القديمة:", repr(e))
